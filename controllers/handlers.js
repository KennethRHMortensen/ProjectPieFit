...
module.exports = {
  ...
  async displayCard(req, res) {
    let r = await models.showCards(req, res);
    let content = "text/html; charset=utf-8";
    let path = "views/createdcards.html";
    getAndServe(res, path, content, {
      createdcards: r,
      a: "right aside",
      b: "left aside",
  ...
  async receiveCard(req, res, data) {
    ...
    module.exports.displayCard(req, res); // go to createdCards page
  },

  // async verifyAdmin(req, res, data) {
  //   let session = cook.cookieObj(req, res); // create session object
  //   let obj = lib.makeWebArrays(req, data); // home made GET and POST objects
  //   let r = await models.verify(obj);
  //   if (r.length == 1 && obj.POST.name === "admin") {
  //     //let name = "" + r[0].name;
